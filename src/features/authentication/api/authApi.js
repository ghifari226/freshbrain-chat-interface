...
import { ROLE_SCOPES } from '@features/access-control'
import {
  ALL_PERMISSIONS,
...
import { aiEngineApi, authHeaders, gatewayApi } from '@integrations/http/httpClient.ts'
...
// Mock token pengembangan ini dihapus setelah gateway autentikasi menyediakan token asli.
async function mintDevToken({ user_id, role, allowed_scopes }, { signal } = {}) {
  const { data } = await aiEngineApi.post('/dev/token', { user_id, role, allowed_scopes }, { signal })
  return data
}
